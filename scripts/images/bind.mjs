...
for (const s of lesson.slides) { delete s.imageSrc; delete s.item; delete s.imageId; for (const b of s.beats || []) delete b.imageSrc; }
const bound = await bindImages(lesson.slides, n, ROOT, { strict });
await writeFile(jsonPath, JSON.stringify(lesson, null, 2) + '\n');
console.log(`✓ bound ${bound} image slot(s) into ${id}.method.json${strict ? ' (strict: approved-only)' : ''}`);
